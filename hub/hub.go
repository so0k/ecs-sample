...
//start listening on subscriber connection
func InitHub() error {
	go func() {
		for {
...
func Subscribe(s *glue.Socket, t string) error {
	...
	log.Println("subscribing socket to topic:",t)
...
func UnsubscribeAll(s *glue.Socket) error {
	...
	log.Println("unsubscribing & deleting socket mapping...")
...
func Emit(t string, m string) error {
	log.Println(fmt.Sprintf("Publishing %s to topic %s",m,t))
...
func EmitLocal(t string, m string) {
	...
	//Emit m to all sockets mapped to t
	log.Println(fmt.Sprintf("Received %s from topic %s",m,t))
